...
const getNodesArray = (node: unknown): ChildNode[] =>
  Array.from((node as Container).nodes ?? []);
...
const transformRuleOrDecl = (child: ChildNode, opts: TransformOpts): Promise<void> => {
...
const transformNode = (node: Container | { nodes: ChildNode[] }, opts: TransformOpts): Promise<void> =>
  waterfall(getNodesArray(node), child =>
    transformRuleOrDecl(child, opts).then(() => {
      if (child.parent && !isDisabledStructural(child, opts)) return transformNode(child as unknown as Container, opts);
    })
  );
